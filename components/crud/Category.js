...
const Category = ()=>{
    ...
    const showCategories = () => {
        return categories.map((cete, i) => {
            return (
                <button
                    onDoubleClick={() => deleteConfirm(cete.slug)}
                    title="Double click to delete"
                    key={i}
                    className="btn btn-outline-primary mr-1 ml-1 mt-3"
                >
    ...
    const deleteCategory = slug => {
        ...
        removeCategory(slug, token).then(data => {//console.log(data)
            if (data.error) {
                console.log(data.error);
            } else {
    ...
    const newCategoryForm=()=>(
        <form onClick={clickSubmit}>
            <div className='form-group'>
                <label className="text-muted">Name</label>
                <input type="text" className='form-controll' onChange={handleChange} value={name} required/>
            </div>
            <div>
