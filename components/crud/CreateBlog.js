...
import {createBlogg } from './../../actions/blog';
...
const CreateBlog =({router})=>{
    ...
    const [body,setBody] =useState(blogFormDataLocalStora())
...
const publishBlog = e => {
    ...
    createBlogg(formData, token).then(data => {
        if (data.error) {
            setValues({ ...values, error: data.error });
        } else {
...
const handleChange = name => e => {
 ...
 const handleBody = e => {
   ...
   const handleToggleTags =t =>()=>{
       setValues({...values,error:''})
       const clickedCheckedTags = checked.indexOf(t)
       const all =[...checked]
  ...
  /*   const publishBlog =e =>{
       ...
       CreateBlog(formData,token).then(data =>{
           if(data.error){
               ...
               setValues({
                   ...values,title:'',error:'',success:'a new blog has created'})
                   setBody('')
                   setCategories([])
                   setTags([])
           }
